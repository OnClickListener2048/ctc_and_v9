        ...
        mConnectionListener = new IYWConnectionListener() {
            ...
            public void onDisconnect(int code, String info) {
                if (code == YWLoginCode.LOGON_FAIL_KICKOFF) {
                    ...
                    //在其它终端登录，当前用户被踢下线
                    normalDialog =
                            new AlertDialog.Builder(MyBaseFragmentActivity.this.mActivity);
                    ...
                    if (!MyBaseFragmentActivity.this.isFinishing()) {
                ...
                if (data.get("status").equals("success")) {
                    ...
                    mLoginService.logout(new IWxCallback() {
                        ...
                        public void onSuccess(Object... arg0) {
                            MyBaseFragmentActivity.this.runOnUiThread(new Runnable() {
                                @Override
                                public void run() {
//									InterfaceMainActivity.this.sToast("退出成功");
                                    LoginUser.getInstantiation(MyBaseFragmentActivity.this.getApplicationContext()).loginOut();
                                    LoginUser.getInstantiation(MyBaseFragmentActivity.this.getApplicationContext()).startLoginActivity(MyBaseFragmentActivity.this);
                                    finish();
                                }
                            });
