...
public class PalyExplainActivity extends MyBaseActivity {
	...
	private void init() {
		...
		fragment_share_title_text.setText("玩法说明");
		...
		if (Tools.getAPNType(PalyExplainActivity.this) == true){
	...
	{
		//mZProgressHUD.show();
		HttpConnect.post(this, "member_options_explain", null, new Callback() {
			@Override
			public void onResponse(Response arg0) throws IOException {
				JSONObject data = JSONObject.fromObject(arg0.body().string());
				if (data.get("status").equals("success")) {
					if( data.optJSONArray("data")!=null)
					{
						final String url = data.optJSONArray("data").optJSONObject(0).optString("value");
...
//						    NewSchoolActivity.this.sToast("暂无数据");
			...
			public void onFailure(Request arg0, IOException arg1) {
				...
				Handler dataHandler = new Handler(getContext().getMainLooper()) {
					@Override
					public void handleMessage(final Message msg) {
//						PalyExplainActivity.this.sToast("请检查您的网络链接");
					}
				};
				dataHandler.sendEmptyMessage(0);
			}
