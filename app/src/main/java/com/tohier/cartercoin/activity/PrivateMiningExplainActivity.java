...
public class PrivateMiningExplainActivity extends MyBaseActivity {
	...
	private void init() {
		...
		fragment_share_title_text.setText("私有矿池说明");
		...
		if (Tools.getAPNType(PrivateMiningExplainActivity.this) == true){
		...
		tv_target_more.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View v) {
				startActivity(new Intent(PrivateMiningExplainActivity.this,RevisionBuyAssetsActivity.class));
				finish();
			}
		});
		...
		HttpConnect.post(this, "member_hide_list", null, new Callback() {
			...
			public void onResponse(Response arg0) throws IOException {
				...
				if (data.get("status").equals("success")) {
					final String min = data.getJSONArray("data").getJSONObject(0).getString("assets");

					PrivateMiningExplainActivity.this.runOnUiThread(new Runnable() {
						@Override
						public void run() {
							if(!TextUtils.isEmpty(min)&&min.equals("0"))
							{
								tv_target_more.setVisibility(View.VISIBLE);
								tv_target_more.setText("立即购买>>");
	...
	{
		//mZProgressHUD.show();
		HttpConnect.post(this, "member_help_mine_private", null, new Callback() {
			@Override
			public void onResponse(Response arg0) throws IOException {
				JSONObject data = JSONObject.fromObject(arg0.body().string());
				if (data.get("status").equals("success")) {
