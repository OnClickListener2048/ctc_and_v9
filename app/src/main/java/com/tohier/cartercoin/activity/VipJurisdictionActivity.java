...
public class VipJurisdictionActivity extends MyBaseActivity {
	...
	private void init() {
		...
		fragment_share_title_text.setText("会员权限说明");
		...
		if (Tools.getAPNType(VipJurisdictionActivity.this) == true){
	...
	{
		//mZProgressHUD.show();
		HttpConnect.post(this, "member_update_introduce", null, new Callback() {
			@Override
			public void onResponse(Response arg0) throws IOException {
				...
				if (data.get("status").equals("success")) {
					...
					Handler dataHandler = new Handler(getContext().getMainLooper()) {
						@Override
						public void handleMessage(final Message msg) {
//						    VipJurisdictionActivity.this.sToast("暂无数据");
						}
					};
					dataHandler.sendEmptyMessage(0);
			...
			public void onFailure(Request arg0, IOException arg1) {
				...
				Handler dataHandler = new Handler(getContext().getMainLooper()) {
					@Override
					public void handleMessage(final Message msg) {
//						VipJurisdictionActivity.this.sToast("请检查您的网络链接");
					}
				};
				dataHandler.sendEmptyMessage(0);
			}
