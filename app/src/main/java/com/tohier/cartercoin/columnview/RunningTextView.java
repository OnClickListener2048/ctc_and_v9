...
public class RunningTextView extends TextView {
	...
	private double nowNumber = 0.0000;// 显示的时间
	...
	private void init() {
		...
		formater = new DecimalFormat("00.0000");// 最多四位小数，而且不够两位整数用0占位。可以通过setFormat再次设置
		handler = new Handler() {
			@Override
...
//				Log.v("nowNumber增加之后的值", nowNumber + "");
	 ...
	 * @param moneyNumber
	 */
	public void playNumber(double moneyNumber,double nowNumber) {
//		if (moneyNumber == 0) {
//			RunningTextView.this.setText("0.0000");
//			return;
//		}
		content = moneyNumber;// 设置最后要显示的数字
//		nowNumber = 0.0000;// 默认都是从0开始动画
		thread_pool.execute(new Runnable() {
			@Override
			public void run() {
				Message msg = handler.obtainMessage();
				double temp = content / frames;
				msg.obj = temp < 0.0001 ? 0.0001 : temp;// 如果每帧的间隔比1小，就设置为1
