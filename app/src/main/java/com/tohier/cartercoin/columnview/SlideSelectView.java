...
public class SlideSelectView extends View {
	...
	//小圆半径
	private static final float RADIU_SMALL = 20;
	//大圆半径
	private static final float RADIU_BIG = 40;
	//线的高度
	private static float HEIGHT_LINE = 10;
	//线距离两头的边距
	private static float MARGEN_LINE = RADIU_BIG * 2.2f;
//	private final Paint mPaint1;
	...
	public SlideSelectView(Context context, AttributeSet attrs) {
		...
		countOfSmallCircle = a.getInt(R.styleable.SlideSelectView_circleCount, 12);
		textSize = a.getInt(R.styleable.SlideSelectView_textSizes, (int) TypedValue.applyDimension(
				TypedValue.COMPLEX_UNIT_SP, 10, getResources().getDisplayMetrics()));
		a.recycle();

		mPaint = new Paint();
		mPaint.setColor(0xfffeb831);
		mPaint.setAntiAlias(true);

//		mPaint1 = new Paint();
//		mPaint1.setColor(0xffff7b03);
//		mPaint1.setAntiAlias(true);
	...
	protected void onDraw(Canvas canvas) {
		...
		try {
			//画文字
			canvas.drawText(text4Rates[currentPosition], circlesX[currentPosition] - 200,
					(mHeight / 2) - RADIU_BIG -
							RADIU_SMALL,
					mTextPaint);
		}catch (Exception e){}
